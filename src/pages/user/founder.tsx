...
export default function Founder() {
  ...
  const fetchUsersAndMeetings = async () => {
    ...
    setUsers(usersWithMeetingCounts.filter(user => user.meetingCount > 0).sort((a, b) => b.meetingCount - a.meetingCount));
  };
            ...
            <tbody>
              {users.map((user, index) => (
                ...
                key={index}>
                  ...
                  <td className="p-4 text-center align-middle">{user.meetingCount}</td>
                </tr>
              ))}
            </tbody>
          </table>
